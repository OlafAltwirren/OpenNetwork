...
local destinationUUID = network.inp.getInterfaceForDomainName(args[1])
if not destinationUUID then
    print "Unknown host."
end

-- guess source UUID for sending.
local sourceUUID = nil --< nil will get sendFrame to choose the appropriate sending interface
...
local function doPing()
    ...
    local icmp_seq = network.icmp.ping(sourceUUID, destinationUUID, payload)
